...
def strategy_random_backtrack(zone, start, end, time_limit=5.0, live=False,
    ...
    def dfs(u, visited, path):
        ...
        # bias toward turning: prefer neighbors not in same direction as previous step
        if len(path) >= 2:
            px, py = path[-2]
            ux, uy = u
            prev_dir = (ux - px, uy - py)
        else:
            prev_dir = None
        def score(n):
            deg_score = sum(1 for nb in adj[n] if nb not in visited)
            if prev_dir is None:
                turn_flag = 0
            else:
                dx = n[0] - u[0]; dy = n[1] - u[1]
                turn_flag = 1 if (dx, dy) == prev_dir else 0  # 0=turn, 1=straight
            return (turn_flag, deg_score)
        neighs.sort(key=score)
        for v in neighs:
            rem = zone - visited - {v}
